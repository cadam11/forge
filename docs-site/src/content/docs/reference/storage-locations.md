...
| The five-per-tab floor is honoured on the daily pass too, counted per tab over pinned and unpinned alike                                                                | `packages/main/src/services/config/query-results-store.ts:48-73` (`floorProtectedIds`), `:339-349` (applied in the `olderThan` branch)              |
