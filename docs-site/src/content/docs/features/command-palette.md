...
owns it. Two commands render this way today: _Server properties_ and _Database properties_.
_Object properties_ and a database's _Delete…_ are unowned as well, but they are sidebar-only
commands the palette never lists (see above), so you meet them in the explorer's menus instead.
...
| 58 commands in the catalogue                                                       | `packages/renderer/src/commands/catalogue.ts:272-803`, `commands/registry.ts:326`                                               |
...
| Server properties and Database properties are palette-visible and unowned          | `packages/renderer/src/commands/catalogue.ts:462-469, 507-514`, `commands/registry.ts:387, 404`, and no `useCommand` for either |
| Object properties and delete-database are unowned but never listed by the palette  | `packages/renderer/src/commands/catalogue.ts:762-769, 786-794`, `commands/registry.ts:471-473`                                  |
