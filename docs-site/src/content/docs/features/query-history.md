...
**Every execute**, from every surface. The main process writes an entry as each statement returns,
with its connection, its database, the SQL, when it ran, how long it took, the row count, and the
error if it failed.

That includes things you did not type: an [execution plan](../execution-plans/) request appears as
the `EXPLAIN` or `SET STATISTICS PROFILE` statement that was really sent, and following a foreign key
in the [row inspector](../results-grid/) appears as the lookup it ran. Both are true records of
...
| Every execute is recorded, with connection, database, SQL, time, duration, rows, error   | `packages/main/src/ipc/query.ipc.ts:40-56`                                                                 |
...
| An FK lookup in the row inspector is recorded too                                        | `packages/renderer/src/features/query/row-detail-panel.tsx:28-40`                                          |
| 1,000 entries are kept, oldest first off                                                 | `packages/main/src/services/config/query-history.ts:17, 60-67`                                             |
| The SQL is capped at 10,000 characters per entry                                         | `packages/main/src/ipc/query.ipc.ts:46`                                                                    |
