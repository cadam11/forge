...
The tooltip calls that setting "maximum rows to display" while **Settings ▸ Query** labels it
"Maximum rows to fetch". They are the same one control.
...
| The tooltip says "maximum rows to display"; the settings row is labelled "Maximum rows to fetch" | `packages/renderer/src/features/query/results-grid.tsx:490-491` vs `features/settings/settings-groups.tsx:323` |
