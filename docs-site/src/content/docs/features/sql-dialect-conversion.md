...
spawns a small local service from `resources/python/sqlglot-server.py` using **`python3` from your
PATH**, on `127.0.0.1` with an ephemeral port, and talks to it over HTTP.
...
| `python3` is not on your PATH                | _Python 3 is required for SQL conversion. Please install Python 3 and ensure "python3" is on your PATH._ |
...
> message even with Python installed. That rough edge is tracked, and
> [Prerequisites](../../getting-started/prerequisites/) says so too.
...
| Conversion runs through a Python sqlglot service spawned as `python3` | `packages/main/src/services/sql/sql-converter.ts:1-11, 94-103`, `sqlglot/sqlglot-client.ts:52-59`        |
