...
| ---------- | ----------------------------------- | ------------------------------------------------------------------------------------ |
| **Menu**   | An Electron menu item (23 bindings) | The keystroke never reaches the page at all — the menu fires and sends the command   |
| **App**    | A key listener in the window (3)    | ⌘J, ⌘P and ⌥⌘S. These must avoid every registered menu accelerator, or the menu wins |
| **Editor** | Monaco itself (1)                   | ⌘E. The menu shows it but deliberately does not bind it, so the editor can           |

That last distinction is why the [snippet library](../snippets/) is on ⌥⌘S and not ⇧⌘S: ⇧⌘S is File
▸ Save Query As, so a window-level listener on it would never have run.
...
| 23 menu bindings, 3 app bindings, 1 editor binding                               | `packages/renderer/src/commands/catalogue.ts:239-242, 272-803`                                                  |
| The app-bound three are ⌘J, ⌘P and ⌥⌘S                                           | `packages/renderer/src/commands/catalogue.ts:559-568, 623-631, 632-642`                                         |
