...
- **The app's PATH is not your shell's PATH.** On macOS, an application launched from the Dock or
  from Finder does not read `~/.zshrc` or `~/.bash_profile`, so a directory you added there — the
  keg-only `mysql-client` bin directory is the usual one — may still be missing after a restart.
  Starting Joinery from a terminal window inherits that shell's environment, which is the quickest
  way to confirm this is what is happening.
...
> **Note** — Homebrew's `postgresql@16` is keg-only until you run `brew link --force
postgresql@16`, and `mysql-client` is keg-only permanently, which is why its install needs an
> explicit PATH line. Both are on the Prerequisites page in full.
